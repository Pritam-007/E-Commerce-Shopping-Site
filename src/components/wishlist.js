...
function WishList() {
    ...
    useEffect(() => {
      axios.get('http://localhost:4500/product/wishlist/mylist/' + useremail)
        .then(response => {
    ...
    function MoveToCart(id,currentrowid,index) {
      ...
      console.log(obj);
      axios.post('http://localhost:4500/product/cart',obj).then(
    ...
    function RemoveFromWishList(currentrowid,index) {
      ...
      axios.delete('http://localhost:4500/product/wishlist/removeitem/'+currentrowid).then(setEmpList(tempemplist)).catch((error) => {
        console.log(error);
      });
